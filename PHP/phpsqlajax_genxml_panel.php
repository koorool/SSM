...
$query = "SELECT marker.lat,marker.lng,marker.Freeware,markerinfo.description,markerinfo.sim FROM marker, markerinfo WHERE marker.name= '$name' AND markerinfo.name = '$name'";
...
while ($row = @mysql_fetch_assoc($result)) {/* && $rowsql = @mysql_fetch_assoc($resultsql)) {*/
	// ADD TO XML DOCUMENT NODE
	$node    = $dom->createElement("marker");
	$newnode = $parnode->appendChild($node);
	//$newnode->setAttribute("name",$row['name']);
	$newnode->setAttribute("description", $row['description']);
	$newnode->setAttribute("sim", $row['sim']);
